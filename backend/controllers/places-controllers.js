...
const getPlaceById = async (req, res, next) => {
    ...
    if (!place) {
        const error = HttpError('Could not find a places for the provided id', 404);
        return next(error);
    }
...
const updatePlace = async (req, res, next) => {
    ...
    if (place.creator.toString() !== req.userData.userId) {
        const error = new HttpError('Unauthorized attempt, you are not allowed to edit this place', 401);
        return next(error);
    }
...
const deletePlace = async (req, res, next) => {
    ...
    if (place.creator.id !== req.userData.userId) {
        const error = new HttpError('Unauthorized attempt, you are not allowed to delete this place', 401);
        return next(error);
    }
